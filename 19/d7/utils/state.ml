...
module Is_monad (M : Monad.S) (S : sig type t end) : Monad.S with type 'a t = S.t -> ('a * S.t) M.t = struct
...
module MakeT (M : Monad.S) (S : sig type t end) = struct
  ...
  let lift (x : 'a M.t) : 'a t =
    fun s -> M.bind x ~f:(fun y -> M.pure (y, s))

  let unlift_fun (f : ('a -> 'b) t) : 'a -> 'b t = 
    fun x s -> 
      M.map (f s) ~f:(fun (f', s') -> (f' x, s'))   
end
